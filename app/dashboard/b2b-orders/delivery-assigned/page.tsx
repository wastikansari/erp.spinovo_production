...
function CopilotCard({ copilot }: { copilot: B2BDeliveryAssignBooking['copilot_details'] }) {
...
function OrderItemsPanel({ item }: { item: B2BDeliveryAssignBooking }) {
...
export default function B2BDeliveryAssignedPage() {
  const [assignedList, setAssignedList] = useState<B2BDeliveryAssignBooking[]>([]);
  ...
  const [reassignTarget, setReassignTarget] = useState<B2BDeliveryAssignBooking | null>(null);
  ...
  const fetchList = useCallback(async (page: number) => {
    try {
      setLoading(true);
      setError('');
      const response = await B2BAssignApiService.getDeliveryAssignedList(page, 20);
      if (response.status && response.data) {
        ...
        setError(response.msg || 'Failed to fetch B2B delivery assigned orders');
        setAssignedList([]);
      }
    } catch {
  ...
  return (
    <div className="space-y-5 p-4 md:p-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">B2B Delivery Assigned Orders</h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            Manage delivery assignments for B2B orders
          </p>
        </div>
        ...
        <CardContent className="p-0">
          ...
          {loading ? (
            ...
            <div className="py-24 text-center">
              ...
              <p className="font-semibold text-lg">No Delivery Assigned Orders</p>
      ...
      <ReassignCopilotForm
        ...
        onReassign={(newCopilotId) =>
          B2BAssignApiService.deliveryReassign({
