...
import { getB2BServiceCategories, updateB2BCategory } from '@/lib/api/b2bService';
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function UpdateB2BCategoryPage() {
  ...
  useEffect(() => {
    ...
    async function fetchCategory() {
      setLoading(true);
      const services = await getB2BServiceCategories();
  ...
  async function handleSave() {
    ...
    const result = await updateB2BCategory(serviceId, categoryId, {
    ...
    return (
      <div className="max-w-xl mx-auto p-6 space-y-4">
        <Button variant="ghost" size="sm" className="gap-2 -ml-2" onClick={() => router.back()}>
          <ArrowLeft className="h-4 w-4" /> Back to B2B Services
        </Button>
        <div className="rounded-xl border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950 p-6 text-center space-y-2">
          ...
          <Button variant="outline" size="sm" className="mt-2" onClick={() => router.push('/dashboard/b2b-services')}>
            Go to B2B Services
          </Button>
        </div>
      </div>
