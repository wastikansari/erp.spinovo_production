...
export default function DeliveryAssignedPage() {
  ...
  const fetchDeliveryAssignedBookings = async (page: number) => {
    try {
      ...
      if (response.status && response.data) {
        ...
        logger.info('Delivery assigned bookings loaded successfully', {
        ...
        setError(errorMsg);
        logger.error('API Error', { message: errorMsg }, 'DeliveryAssignedPage');
        toast({
          title: 'Error',
      ...
      const handledError = errorHandler.handle(error as Error, 'DeliveryAssignedPage.fetchDeliveryAssignedBookings');
      setError(handledError.message);
      logger.error('Failed to fetch assigned bookings', { error: handledError }, 'DeliveryAssignedPage');
      toast({
        title: 'Error',
  ...
  useEffect(() => {
    fetchDeliveryAssignedBookings(currentPage);
  }, [currentPage]);
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-3xl font-bold tracking-tight">Delivery Assigned Orders</h1>
        </div>
        <Card>
  ...
  return (
    <ErrorBoundary>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-3xl font-bold tracking-tight">Delivery Assigned</h1>
        ...
        <Card>
          <CardHeader>
            ...
            <div className="text-sm text-muted-foreground">
              Total Delivery Orders: {totalCount}
            </div>
          </CardHeader>
          <CardContent>
            {error && (
