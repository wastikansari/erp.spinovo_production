...
// B2B has no per-vendor service-capability flags yet (v1 scope) — unlike
// retail's ProcessAssignForm, we don't filter by vendorMatchesService, just
...
interface B2BProcessAssignFormProps {
    open: boolean;
...
export function B2BProcessAssignForm({
    open,
    onOpenChange,
    orderId,
    orderNo,
    onSuccess,
}: B2BProcessAssignFormProps) {
    ...
    const fetchVendors = async () => {
        ...
        try {
            ...
            if (res.status && res.data) {
                const list = res.data.vendorList.filter((v) => v.accountIsActive === true);
                list.sort((a, b) => {
    ...
    const handleAssign = async () => {
        if (!selectedVendorId || !orderId) return;
        setSubmitting(true);
        try {
            const res = await B2BAssignApiService.processAssign({
                b2b_order_id: orderId,
                vendor_id: selectedVendorId,
            });
            if (res.status) {
                toast({ title: 'Assigned', description: 'B2B order assigned to vendor successfully.' });
                onOpenChange(false);
                onSuccess();
            } else {
                    ...
                    <DialogDescription className="text-xs mt-1">
                        ...
                        {loadingVendors ? 'Loading vendors…' : (
                            <>
                                ...
                                {' · '}
                                {vendors.length} total active vendors
                            </>
                        )}
                    </DialogDescription>
                </DialogHeader>
